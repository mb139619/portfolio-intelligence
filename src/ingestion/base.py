...
class IngestionResult:
    ...
    def __str__(self) -> str:
        if self.success:
            return (f"[OK] {self.source}/{self.identifier}: {self.rows} rows "
...
class BaseIngester(ABC):
    ...
    def validate(self, df: pl.DataFrame) -> pl.DataFrame:
        """Drop rows where all non-date columns are null. Override per source."""
    ...
    def ingest(self, identifier: str, start: str,
               end: str | None = None) -> tuple[pl.DataFrame, IngestionResult]:
        try:
