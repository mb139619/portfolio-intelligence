...
ASSET_CLASS_OUTLIER_BANDS: dict[str, tuple[float, float]] = {
    "fixed_income": (0.05, 0.12),   # govvies/IG barely move; >12% is a data error
...
class QualityFinding:
    ...
    def __str__(self) -> str:
        return (f"[{self.severity.value.upper():8}] {self.ticker} · "
...
def check_short_history(
    df: pl.DataFrame, ticker: str, min_obs: int = 252
) -> list[QualityFinding]:
...
def check_return_outliers(
    ...
    sev = (Severity.CRITICAL if (abs_rets[flagged_idx] > crit_band).any()
    ...
    more = ("" if len(events) <= message_sample
            else f", +{len(events) - message_sample} more")
